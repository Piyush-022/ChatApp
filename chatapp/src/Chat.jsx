...
export const Chat = () => {
  ...
  useEffect(() => {
    const getuser = async () => {
      ...
      const offpeople = data
        .filter((p) => p._id !== id)
        .filter((p) => onlinepeople[p._id] == null);
      const offpplobj = {};
    ...
    <div className="flex h-screen">
      <div className="relative w-1/3 bg-white">
        ...
        <div className="absolute bottom-0 flex justify-center w-full p-2 mx-auto ">
          <button
            onClick={logout}
            className="p-2 text-black transition-all bg-blue-100 border rounded-sm shadow-sm hover:bg-black hover:text-white "
          >
            Logout
          </button>
        </div>
